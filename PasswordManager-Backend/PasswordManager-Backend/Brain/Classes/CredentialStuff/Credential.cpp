...
unsigned long long cCredentialStuff::GetShift(const std::wstring master_password)
{
...
{
    const auto cred = new cCred(L"", L"", L"");
...
std::wstring cCredentialStuff::CppCredToPython(const cCred c)
{
    const auto nameLength = std::to_wstring(c.Name.length());
    const auto emailLength = std::to_wstring(c.Email.length());
    const auto passwordLength = std::to_wstring(c.Password.length());

    const auto name = c.Name;
    const auto email = c.Email;
    const auto password = c.Password;

    return nameLength + L"," + emailLength + L"," + passwordLength + L" " + name + email + password;
}
...
std::wstring cCredentialStuff::DecryptCppCred(const std::wstring cred_python, const std::vector<unsigned long long> key, const unsigned long long shift, const std::wstring pass_phrase)
{
    ...
    newCred->Name = cDecrypt::DecryptString(credFromPython.Name, key, shift, pass_phrase);
    newCred->Email = cDecrypt::DecryptString(credFromPython.Email, key, shift, pass_phrase);
    newCred->Password = cDecrypt::DecryptString(credFromPython.Password, key, shift, pass_phrase);
